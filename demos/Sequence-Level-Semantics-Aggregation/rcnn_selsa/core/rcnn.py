...
def sample_rois(rois, fg_rois_per_image, rois_per_image, num_classes, cfg,
                labels=None, overlaps=None, bbox_targets=None, gt_boxes=None, output_sort=False):
    """
    ...
    if output_sort:
        _overlaps = overlaps[fg_indexes]
        _order = np.argsort(_overlaps)[::-1]
        fg_indexes = fg_indexes[_order]
