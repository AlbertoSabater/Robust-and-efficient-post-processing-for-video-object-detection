...
def train_net(args, ctx, pretrained, epoch, prefix, begin_epoch, end_epoch, lr, lr_step):
    ...
    # infer max shape
    H = max([v[0] for v in config.SCALES])
    W = max([v[1] for v in config.SCALES])
    max_data_shape = [('data', (config.TRAIN.BATCH_IMAGES, 3, H, W)),
                      ('data_bef', (config.TRAIN.BATCH_IMAGES, 3, H, W)),
                      ('data_aft', (config.TRAIN.BATCH_IMAGES, 3, H, W))]
    ...
    if config.network.dtype == 'float16':
        rescale_grad_factor = 1.0 / 128.0
        multi_precision = True
    else:
        rescale_grad_factor = 1.0
    ...
    optimizer_params = {'momentum': config.TRAIN.momentum,
                        ...
                        'rescale_grad': 1.0 * rescale_grad_factor,
                        'clip_gradient': None,
